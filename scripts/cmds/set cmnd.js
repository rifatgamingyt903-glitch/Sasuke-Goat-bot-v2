module.exports = {
  ...
  onStart: async function ({ args, event, api, usersData }) {
    const permission = ["100051869042398", "61574316459995"];
  if (!permission.includes(event.senderID)) {
    api.sendMessage("You don't have enough permission to use this command. Only Admin Can Use It.", event.threadID, event.messageID);
    return;
  }
    const query = args[0];
    ...
    if (query.toLowerCase() === 'exp') {
      ...
      return api.sendMessage(`Set balance to ${amount} for ${name}.`, threadID);
    } else {
      return api.sendMessage("Invalid query. Use 'exp' to set experience points or 'money' to set balance ", threadID);
    }
  }
};
